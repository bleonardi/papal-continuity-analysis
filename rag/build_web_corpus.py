"""Build a lean, document-level corpus JSON for the client-side chat demo.

Unlike build_index.py (which produces overlapping chunks + embeddings for
server-side semantic search), this ships one entry per source document with
no embeddings — the static page indexes it in-browser with MiniSearch
(lexical search), so there's no model to download and no vector math to run
client-side.
"""
...
DATA_DIR = ROOT / "data"
OUT_PATH = Path(__file__).resolve().parent / "web" / "corpus.json"
